...
class CartItemView(TemplateView):
    ...
    def get_formset(self, clear=False):
        ...
        if session_key:
            if clear:
                formset = CartItemFormSet(queryset=CartItem.objects.filter(cart_id=session_key))
            else:
                formset = CartItemFormSet(queryset=CartItem.objects.filter(cart_id=session_key), data=self.request.POST or None)
        else:
